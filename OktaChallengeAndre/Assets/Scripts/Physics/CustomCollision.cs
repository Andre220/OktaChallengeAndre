...
public class CustomCollision : ICustomCollision
{
    ...
    public event Action OnCollisionHappened;

    #region definições da classe e suas propriedadesç
    ...
    public void RemoveColliderToBuffer(BasicPhysicsObject bpo)
    {
        collidersInScene.Remove(bpo);
    }
    #endregion

    public void OnCollision(BasicPhysicsObject callerCollider, CollisionType colType)
    {
        ...
        {
            //if (currentCollider.gameObject.GetInstanceID() != callerCollider.GetInstanceID())
            //{
            //    switch (colType)
            //    {
            //        case CollisionType.CircleAndCircle:
            //            CollisionBetweenCircleAndCircle((CircleModel)callerCollider, (CircleModel)currentCollider);
            //            break;
            //        case CollisionType.CircleAndSquare:
            //            CollisionBetweenCircleAndCircle((CircleModel)callerCollider, (CircleModel)currentCollider);
            //            break;
            //        case CollisionType.SquareAndSquare:
            //            CollisionBetweenCircleAndCircle((SquareModel)callerCollider, (SquareModel)currentCollider);
            //            break;
            //    }
            //}
        }
    ...
    public void CollisionBetweenCircleAndSquare(CircleModel circle, SquareModel square)
    {
        ...
        if (Vector2.Distance(circle.transform.position, square.transform.position) < minDistanceTest)
        {
                    ...
                    y - circle.CircleRadius < square.TopEdge && square.BottomEdge < y + circle.CircleRadius))
            {
                ...
                else if (circle.stopOnCollide || square.stopOnCollide)
                {
    ...
    public void CollisionBetweenCircleAndCircle(CircleModel circleA, CircleModel circleB)
    {
        ...
        if (distanceBetweenCircles < minDistanceTest)
        {
    ...
    public void CollisionBetweenSquareAndSquare(SquareModel squareA, SquareModel squareB)
    {
        ...
        if (Vector2.Distance(squareA.transform.position, squareB.transform.position) < minDistanceTest)
        {
    ...
    public void StopOnCollide(BasicPhysicsObject toStop, BasicPhysicsObject other) // Faz o objeto parar de se mover caso colida
    {
        ...
        toStop.transform.position += new Vector3(0, Mathf.Cos(angle) * distanceBetweenCircles / 2, 0);
    }
    #endregion
}
